...
@section('content')
                            ...
                            <div class="bg-yellow-100 rounded-lg p-3">
                                ...
                                <div class="text-sm text-yellow-800">
                                    <div class="flex justify-between mb-1">
                                        ...
                                        <span
                                            class="font-medium">Expires: {{ $purchaseCheck['active_subscription']->expires_at->format('M j, Y g:i A') }}</span>
                                    </div>
                                    <div class="text-xs">
                                        Time
                                        remaining: {{ $purchaseCheck['active_subscription']->expires_at->diffForHumans() }}
                                    </div>
                                </div>
                            </div>
                        @endif
            ...
            @empty
                <div class="bg-white rounded-xl card-shadow p-8 text-center">
                    ...
                    @if(isset($settings) && $settings->whatsapp_support_enabled)
                        <div class="mt-4">
                            <a href="{{ SettingsHelper::getWhatsAppUrl('Need help with packages') }}"
                               target="_blank"
    ...
    @push('scripts')
        <script>
            // Pre-define currentPackage only if user has active subscription
            function selectPackage(packageId, packageName, packagePrice) {
                @if(isset($purchaseCheck) && $purchaseCheck['has_active'])
                // Inline JS string interpolation for currentPackage
                const currentPackage = {
                    ...
                    expires_at: "{{ isset($purchaseCheck['active_subscription']) && $purchaseCheck['active_subscription']->expires_at ? $purchaseCheck['active_subscription']->expires_at->format('c') : '' }}"
                };
                ...
                <div class="p-6">
                    ...
                    <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
                        <p class="text-sm text-yellow-800">
                            ...
                            <strong>Important:</strong> Your current subscription will be immediately deactivated and
                            replaced.
    ...
    @push('scripts')
        <script>
            ...
            function showUpgradeModal(packageId, packageName, packagePrice, currentPackage) {
                ...
                // Defensive: fallback if currentPackage is missing
                const safeCurrentPackage = currentPackage || {
                    name: 'Unknown',
                    price: '0',
                ...
                const currentInfo = `
            <div class="space-y-1">
                <div><strong>Package:</strong> ${safeCurrentPackage.name}</div>
                <div><strong>Price:</strong> GH₵${parseFloat(safeCurrentPackage.price).toFixed(2)}</div>
                <div><strong>Data:</strong> ${safeCurrentPackage.data_limit || 'Unlimited'}</div>
                <div><strong>Expires:</strong> ${safeCurrentPackage.expires_at ? new Date(safeCurrentPackage.expires_at).toLocaleDateString() : 'N/A'}</div>
            </div>
        `;
...
<div id="registrationModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden">
    <div class="bg-white rounded-xl p-6 max-w-md w-full mx-4">
        <div class="text-center">
            ...
            <p class="text-gray-600 mb-6">Your phone number is not registered. You need to register first before you can
                purchase a package.</p>
