...
class Customer extends Model
{
    ...
    /**
     * Check if customer has any subscriptions (active, expired, or pending)
     */
    ...
    /**
     * Check if customer has never had any subscriptions
     */
    public function isCompletelyUnsubscribed()
    {
        return !$this->hasAnySubscription();
    }

    /**
     * Check if customer has expired subscriptions but no active ones
     */
    public function hasExpiredSubscriptionsOnly()
    {
        return $this->hasAnySubscription() && !$this->hasActiveSubscription();
    }

    /**
     * Get customer's subscription status for RADIUS access control
     */
    public function getSubscriptionStatus()
    {
        if ($this->hasActiveSubscription()) {
            return 'active';
        } elseif ($this->hasAnySubscription()) {
            return 'expired';
        } else {
            return 'unsubscribed';
        }
     ...
     */
    public function applyRadiusAccess()
    {
        $status = $this->getSubscriptionStatus();
        
        switch ($status) {
            case 'active':
                // Full internet access with subscription limits
                $activeSubscription = $this->getActiveSubscription();
                if ($activeSubscription) {
                    $activeSubscription->syncRadiusStatus();
                }
                break;
                
            case 'expired':
                // Redirect to portal for renewal
                \App\Models\RadCheck::redirectUserForExpiration($this->username);
                break;
                
            case 'unsubscribed':
                // Portal-only access for 5 minutes
                \App\Models\RadCheck::setPortalOnlyMode($this->username);
                break;
        }
    ...
    /**
     * Handle cleanup when all subscriptions are removed/cancelled
     */
    public function handleUnsubscribe()
    {
        // If customer no longer has any subscriptions, apply portal-only access
        if ($this->isCompletelyUnsubscribed()) {
            \App\Models\RadCheck::setPortalOnlyMode($this->username);
            
            // Log the unsubscribe action
            \Log::info('Customer unsubscribed - applied portal-only access', [
                'customer_id' => $this->id,
    ...
    /**
     * Clean up RADIUS access completely (for customer deletion)
     */
    public function cleanupRadiusAccess()
    {
        // Remove all RADIUS check attributes
        \App\Models\RadCheck::where('username', $this->username)->delete();
        
        // Remove all RADIUS reply attributes  
        \App\Models\RadReply::cleanupUser($this->username);
        
        // Remove from all MikroTik address lists
        \App\Models\RadReply::removeFromAllAccessLists($this->username);
